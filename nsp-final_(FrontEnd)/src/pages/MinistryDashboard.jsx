...
const StudentBadge = ({ s }) => {
  const c = s === 'MinistryPending' ? 'bg-yellow-100 text-yellow-700' : 
            s === 'Approved' ? 'bg-green-100 text-green-700' : 
            'bg-red-100 text-red-700'
  const label = s === 'Approved' ? 'Scholarship Granted' : s === 'MinistryPending' ? 'Pending Review' : s
  return <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${c}`}>{label}</span>
}
const InstBadge = ({ s }) => {
  const c = s === 'Pending' || s === 'Forwarded to Ministry'
...
function MinistryDashboard() {
  ...
  const loadInstituteApps = async () => {
    setLoadingInst(true)
    setError(null)
    try {
      const res = await fetch('/api/auth/officer/ministry/institute-applications', { credentials: 'include' })
      const data = await res.json().catch(() => ({}))
      if (!res.ok) throw new Error(data.error || `Failed to load (${res.status})`)
      setInstApps((data.apps || []).map(app => ({
        ...app,
        id: app._id,
        status: app.status === 'StatePending' ? 'Forwarded to Ministry' : app.status,
      })))
    } catch (err) {
      setError(err.message || 'Failed to load institute applications')
    } finally {
  ...
  useEffect(() => {
    if (tab === 'students') {
      setLoadingInst(true);
      fetch('/api/auth/officer/ministry/scholarship-applications', { credentials: 'include' })
        .then(res => res.json())
        .then(data => setStudentApps(data.apps || []))
        .finally(() => setLoadingInst(false));
    }
    if (tab === 'institutes') loadInstituteApps()
  }, [tab])
  ...
  const handleStudentAction = async (id, action) => {
    try {
      const res = await fetch(`/api/auth/officer/scholarship/${id}/decision`, {
        ...
        credentials: 'include',
        body: JSON.stringify({ decision: action === 'grant' ? 'approve' : 'reject' }),
      });
      if (!res.ok) throw new Error('Action failed');
      alert(action === 'grant' ? "Scholarship granted! Status updated to Approved." : 'Application rejected.');
      // Refresh list
      const listRes = await fetch('/api/auth/officer/ministry/scholarship-applications', { credentials: 'include' });
      const listData = await listRes.json();
      setStudentApps(listData.apps || []);
    } catch (err) {
      alert(err.message);
    }
  }
  const handleInstAction = async (id, action) => {
    try {
      ...
      setInstApps(prev => prev.map(a => a.id === id ? { ...a, status: data.status === 'Approved' ? 'Approved' : 'Rejected' } : a))
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Navbar userType="ministry" onLogout={null} />
                  ...
                  <div key={app._id} className="border rounded p-3 bg-white">
                    <div className="flex justify-between items-start gap-2 mb-1">
                      <div>
                        ...
                        <p className="text-xs text-gray-400">{app.studentId?.state} · {app.appId}</p>
                        <p className="text-xs font-semibold text-green-700 mt-0.5">₹{app.data?.tuitionFee || '0'}</p>
                      </div>
                      <StudentBadge s={app.status} />
                    </div>
                    {app.status === 'MinistryPending' && (
                  ...
                  <tbody>
                    {studentApps.map((app, i) => (
                      <tr key={app._id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        ...
                        <td className="px-3 py-2 text-xs">{app.studentId?.state}</td>
                        <td className="px-3 py-2 text-xs font-semibold text-green-700">₹{app.data?.tuitionFee}</td>
          ...
          {tab === 'institutes' && (
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                ...
                <button className="btn-primary text-xs px-4 py-1 w-full sm:w-auto" onClick={loadInstituteApps}>Refresh</button>
              </div>

              {loadingInst && <p className="text-sm py-4">Loading applications...</p>}
              ...
              <div className="sm:hidden space-y-3">
                {instApps.map(app => (
                  <div key={app.id} className="border rounded p-3 bg-white">
                    ...
                    {app.status === 'Forwarded to Ministry' && (
                      <div className="flex gap-2 mt-2">
